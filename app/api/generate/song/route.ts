...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const out = await generateSong(body.prompt, body.model);
    void meter({ uid: ctx!.uid, kind: 'song' });
    return Response.json(out);
  } catch (e) {
    return json(502, e instanceof Error ? e.message : 'Song generation failed');
  }
}
